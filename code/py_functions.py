...
# Jit function using parallel
@jit(nopython=True, parallel=True)
def decision_single_cell_jit_parallel(ind,
         index,
         EAI,
         Exp,
         nd,
    ...
    # If there is exposure, find the Bayes optimal decision
        ...
        # calculate cost of each decision option for each of the 1000 GAM samples of risk
        cost = np.empty((nd,1000))
        for j in prange(nd): # loop over number of decisions [do this in parallel?]
            for k in prange(1000): # loop over GAM samples
                EAI_k = (10**xi[k] - 1)
                # cost outcome for decision j and sample k
                # QUESTION: Check that I'm converting from EAI to risk properly
                cost[j,k] = decision_inputs[j, 0]*ppl + cost_per_day*(1-decision_inputs[j, 1])*EAI_k
...
def write_decision_file_jit(output_data_path,
                        overwrite,
                        ind,
                        lat,
                        lon,
    ...
    opd = np.empty(nloc)
    for i in range(nloc): # 1711 locations
        opd_i, exp_util_i, util_scores_i, cost_i = decision_single_cell_jit(ind, i, EAI, Exp, nd, decision_inputs, cost_per_day, cweights)
        opd[i] = opd_i
...
def write_decision_file_jit_parallel(output_data_path,
                        overwrite,
                        ind,
                        lat,
                        lon,
    ...
    nloc = ind[0].shape[0]
    opd = np.empty(nloc)
    for i in prange(nloc): # 1711 locations
        opd_i, exp_util_i, util_scores_i, cost_i = decision_single_cell_jit_parallel(ind, i, EAI, Exp, nd, decision_inputs, cost_per_day, cweights)
        opd[i] = opd_i
