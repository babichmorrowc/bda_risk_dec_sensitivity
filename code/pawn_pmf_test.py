...
Y_lon = Y[:,241]
# np.unique(Y_loc, return_counts = True)[1] / len(Y_loc)
Y_scot = Y[:,1445] # location with the highest sensitivity to SSP

# Test out the PAWN pmf function
test_median, test_mean, test_max = PAWN_pmf.pawn_pmf_indices(X = X_numeric, Y = Y_lon, n = 10)
test_median

# Test out the PAWN pmf function with bootstrapping
boot_test_median, boot_test_mean, boot_test_max = PAWN_pmf.pawn_pmf_indices(X = X_numeric, Y = Y_lon, n = 10, Nboot=50)
boot_test_median

#########################################################################
# Test out PMF plotting
YF, fU, fC, xc = PAWN_pmf.pawn_plot_pmf(X = X_numeric,
                                        Y = Y_lon,
                                        n_col=4,
                                        n = 10,
